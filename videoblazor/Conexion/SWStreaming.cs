...
namespace videoblazor.Conexion
{
    public class SWStreaming
    {
        ...
        public SWStreaming()
        {
            var channel = GrpcChannel.ForAddress("http://localhost:8080"); // Cambia la dirección según tu servidor gRPC
            _client = new VideoServiceClient(channel);
        }

        public async Task<Stream> GetVideoStreamAsync(string videoName)
        {
